...
<?php
// Menü nach Kategorie gruppieren
$byCat = [];
...
foreach ($byCat as $cat => $items): ?>
  ...
  <?php foreach ($items as $it): ?>
    <div class="card">
      <div class="row"><strong><?= htmlspecialchars($it['name']) ?></strong>
        <span class="badge"><?= number_format($it['price_cents']/100, 2, ',', '.') ?> €</span></div>
      <div class="row">
...
<script>
const tableCode = <?= json_encode($tbl['code']) ?>;
const cart = {}; // key: id|note -> qty
...
const prices = Object.fromEntries(<?=
  // kleines Preismap fürs Frontend
  json_encode(array_map(fn($m)=>[(int)$m['id'], (int)$m['price_cents']], $menu))
?>.map(v=>v));

const names = Object.fromEntries(<?=
  json_encode(array_map(fn($m)=>[(int)$m['id'], $m['name']], $menu))
?>.map(v=>v));
...
document.getElementById('send').onclick = async ()=>{
  const items = Object.entries(cart).map(([key, qty])=>{
    const [id, note] = key.split('|'); return {item_id: parseInt(id), qty, notes: note||null};
  });
