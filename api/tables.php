<?php
// /api/tables.php — CRUD für Tische (code;name)
// Methoden:
...
//
// Auth: erlaubt ist, wenn EINE der Session-Flags gesetzt ist:
...
// ---- Zugriffskontrolle: Admin ODER Staff ----
...
// ---- CSV-Helfer (falls nicht schon in functions.php vorhanden) ----
...
function unique_code($rows, $tries = 500) {
  $set = [];
  foreach ($rows as $t) { if (isset($t['code'])) $set[$t['code']] = true; }
  while ($tries-- > 0) {
    $c = random_code(8);
    if (empty($set[$c])) return $c;
  }
  return 'T' . dechex(time()); // Fallback
}
...
// ---- Datei & Header sicherstellen (code;name) ----
...
try {
  if ($method === 'GET') {
    $lock = tables_lock();
    $rows = csv_read_assoc(CSV_TABLES); // erwartet header code;name
    tables_unlock($lock);

    // Normalisieren (falls Datei historisch mal andere Header hatte)
    $out = [];
    foreach ($rows as $r) {
      $out[] = [
